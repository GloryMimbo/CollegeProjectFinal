<?php
	...
	session_start();
	$pageNo = "1";
	$pageTitle = "Change of Passport List";
	...
	if(request_get("job")=="delete"){
		...
		$sql = "DELETE FROM `change_of_passport` WHERE id = '".$mid."' ";
		$res = mysql_query($sql) or die("Select change_of_passport Error : ".mysql_error());	
		header("location:change_of_passport_list.php");
		exit;
	}
	...
	{
		$sql = " SELECT * FROM `change_of_passport` WHERE reg_profile_id = '$reg_profile_id' ";
	}
	else if($usertype == "1") // Police
	{
		$sql = " SELECT * FROM `change_of_passport` ";
	}
...
function teamDelete(mid)
{
	...
	if(res==true)
	{
		window.location.href="change_of_passport_list.php?job=delete&del_id="+mid;	
	}
}
</script>